...
import com.atomscat.freeswitch.esl.builder.Command;
...
abstract class AbstractInboundClientCommand extends AbstractInboundClient {
    ...
    public String answer(String address, String uuid) {
        String command = Command.cmd(Command.UUID_ANSWER).arg(uuid).toString();
        if (debugEnabled) {
    ...
    public String bridge(String address, String uuid, String otherUuid) {
        String command = Command.cmd(Command.UUID_BRIDGE).arg(uuid).arg(otherUuid).toString();
        if (debugEnabled) {
    ...
    public String broadcast(String address, String uuid, String path, String smf) {
        String command = Command.cmd(Command.UUID_BROADCAST).arg(uuid).arg(path).arg(smf).toString();
        if (debugEnabled) {
    ...
    public String break0(String address, String uuid, boolean all) {
        String command = Command.cmd(Command.UUID_BREAK).arg(uuid).arg(all ? "all" : null).toString();
        if (debugEnabled) {
    ...
    public String hold(String address, String smf, String uuid, boolean display) {
        String command = Command.cmd(Command.UUID_HOLD).arg(smf).arg(uuid).arg(display ? "all" : EMPTY).toString();
        if (debugEnabled) {
    ...
    public List<String> getVar(String address, String uuid, String var) {
        String command = Command.cmd(Command.UUID_GETVAR).arg(uuid).arg(var).toString();
        if (debugEnabled) {
    ...
    public String setVar(String address, String uuid, String var, String val) {
        String command = Command.cmd(Command.UUID_SETVAR).arg(uuid).arg(var).arg(val).toString();
        if (debugEnabled) {
    ...
    public String record(String address, String uuid, String action, String path, int limit) {
        String command = Command.cmd(Command.UUID_RECORD).arg(uuid).arg(action).arg(path).arg(limit < 1 ? null : String.valueOf(limit)).toString();
        if (debugEnabled) {
    ...
    public String transfer(String address, String uuid, String smf, String dest, String dialplan, String context) {
        String command = Command.cmd(Command.UUID_TRANSFER).arg(uuid).arg(smf).arg(dest).arg(dialplan).arg(context).toString();
        if (debugEnabled) {
